...
import { FaCheckCircle, FaUser, FaShieldAlt, FaExclamationTriangle, FaFileAlt, FaEnvelope } from 'react-icons/fa';
...
const Roles = () => {
  ...
  const newAdminRequests = [
    { name: 'Esthera Jackson', email: 'esthera@email.com', phone: '+1123456789', image: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=150&h=150&fit=crop' },
    { name: 'Esthera Jackson', email: 'esthera@email.com', phone: '+1123456789', image: 'https://images.unsplash.com/photo-1559839734-2b71ea197ec2?w=150&h=150&fit=crop' },
    { name: 'Esthera Jackson', email: 'esthera@email.com', phone: '+1123456789', image: 'https://images.unsplash.com/photo-1599566150163-29194dcaad36?w=150&h=150&fit=crop' },
  ];
    ...
    <section>
      ...
      <div className="flex gap-6 mb-8">
        ...
        <div className="bg-white rounded-[15px] shadow-md p-6 basis-5/12">
          ...
          <div className="space-y-4">
            {newAdminRequests.map((request, index) => (
              <div key={index} className="flex items-center justify-between  hover:bg-gray-100 cursor-pointer p-4 rounded-[15px]">
                <div className="flex items-center gap-3">
                  <img
                    src={request.image}
                    alt={request.name}
                  ...
                  />
                  <div>
                    <p className="font-medium text-base text-gray-900">{request.name}</p>
                    <p className="text-sm text-gray-500">{request.email}</p>
                  </div>
                </div>
                <button
        ...
        <div className="space-y-4">
          {suspiciousActivities.map((activity, index) => (
            <div
              key={index}
              className={`flex items-center justify-between p-4 rounded-[10px] border-l-4 ${
                activity.color === 'red' ? 'border-red-500 bg-[#FEF2F2]' : 'border-yellow-500 bg-yellow-50'
              }`}
            >
              <div className="flex items-center gap-3">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium text-gray-900">{selectedAdminRequest.name}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Email</p>
                  <p className="font-medium text-gray-900">{selectedAdminRequest.email}</p>
                </div>
              </div>
              <div>
                <p className="text-sm text-gray-600">Phone Number</p>
                <p className="font-medium text-gray-900">{selectedAdminRequest.phone}</p>
              </div>
              <div>
          ...
          {selectedTherapistCheck && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-sm text-gray-600">Name</p>
                  <p className="font-medium text-gray-900">{selectedTherapistCheck.name}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Specialty</p>
                  <p className="font-medium text-gray-900">{selectedTherapistCheck.specialty}</p>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-sm text-gray-600">Phone</p>
                  <p className="font-medium text-gray-900">{selectedTherapistCheck.phone}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Email</p>
                  <p className="font-medium text-gray-900">{selectedTherapistCheck.email}</p>
                </div>
              </div>
              <div>
                <p className="text-base text-gray-600 font-medium mb-2">Documents Verified</p>
                {selectedTherapistCheck.documents.map((doc, index) => (
                  <div key={index} className="bg-gray-100 rounded-[10px] p-3 mb-2 flex items-center gap-3  border-2 border-[#B28D28]/10 bg-[#FAE08C]/30">
                    <FaFileAlt className="text-gray-500" size={16} />
                    <div>
                      <p className="text-sm text-gray-900 font-medium">{doc.type}</p>
              ...
              <div>
                <p className="text-base text-gray-900 font-medium mb-2">Documents Expiring</p>
                {selectedTherapistCheck.expiringDocuments.map((doc, index) => (
                  <div key={index} className=" rounded-[10px] p-3 mb-2 flex items-center gap-3 border-2 border-[#B28D28]/10 bg-[#FAE08C]/30">
                    <FaFileAlt className="text-gray-500" size={16} />
                    <div>
                      <p className="text-sm text-gray-900 font-medium">{doc.type}</p>
                      <p className="text-xs text-gray-500">{doc.file} ({doc.size})</p>
