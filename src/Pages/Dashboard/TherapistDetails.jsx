...
const TherapistDetails = () => {
  ...
  const {data:therapistInfo, isLoading, isError, error} = useGetTherapistDetailsInfoQuery(id);
            ...
            <div className="bg-[#B28D28] rounded-[15px] p-6 text-white relative overflow-hidden h-[40vh]">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-4 sm:gap-6 mb-6">
                  ...
                  />
                  <div>
                    <h2 className="text-xl sm:text-2xl font-semibold">{therapistInfo?.therapist_info?.full_name}</h2>
                    <p><span className='font-semibold'>Joined</span> {therapistInfo?.therapist_info?.join_date}</p>
                    <p className="">
                     
                      <span className="ms-2 flex items-center  gap-1  text-white font-semibold">{therapistInfo?.therapist_info?.rating}  <FaStar className='text-[#FFBE0C]'/> </span>
                ...
                <div className="space-y-3 py-10 ms-1">
                  <p className="flex items-end gap-3">
                    ...
                    <p className="text-sm sm:text-base font-medium">{therapistInfo?.therapist_info?.role}</p>
                  </p>
                  <p className="flex items-center gap-3">
                    <FaPhoneAlt size={18} className="text-white" />
                    <span className="text-sm sm:text-base font-medium ms-3">{therapistInfo?.therapist_info?.phone}</span>
                  </p>
                  <p className="flex items-center gap-3">
                    <FaEnvelope size={18} className="text-white" />
                    <span className="text-sm sm:text-base font-medium ms-3">{therapistInfo?.therapist_info?.email}</span>
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10">
            <div className="bg-white flex items-center justify-between rounded-[15px] shadow-md p-6 hover:shadow-lg transition-shadow cursor-pointer">
              <div className="space-y-2">
                ...
                <h1 className="font-bold text-xl text-black">{therapistInfo?.therapist_info?.total_bookings}</h1>
              </div>
            ...
            <div className="bg-white flex items-center justify-between rounded-[15px] shadow-md p-6 hover:shadow-lg transition-shadow cursor-pointer">
              <div className="space-y-2">
                ...
                <h1 className="font-bold text-xl text-black">${therapistInfo?.therapist_info?.total_earnings.toLocaleString()}</h1>
              </div>
              <div className="bg-[#B28D28] p-2 rounded-xl">
  ...
  <div className="space-y-4">
    {therapistInfo?.documents?.length > 0 &&
      therapistInfo.documents.map((docItem, docIndex) => (
        <div key={docIndex}>
          {Object.entries(docItem).map(([key, value], index) => (
            value && (
              <div
                key={index}
    ...
    {[
      ...
      if (!schedule) {
        return (
          ...
          >
            <span className="text-gray-600 font-semibold">{displayDay}</span>
            <span className="text-red-500">OFF</span>
          </div>
        );
      }

      const startTime = schedule.start.slice(0, 5); // HH:MM
      ...
      therapistInfo.reviews.map((review, index) => (
        <div key={index} className="flex gap-4 border p-4 rounded-[15px]">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between gap-2 mb-1">
              <div>
                <span className="font-medium block">{review.client_name}</span>
                <div className="flex">
                  {[...Array(5)].map((_, i) => (
                    <FaStar
                      key={i}
